...
if "!gmode!"=="1" (
    ...
    set /a b_idx=0
    for /f "tokens=1" %%B in ("!bullet_count!") do (
        for /l %%i in (1,1,%%B) do (
            set /a bx=!bullet_x[%%i]!
            set /a by=!bullet_y[%%i]!
                ...
                for /l %%s in (0,1,2) do (
                    ...
                    if "!hit!"=="0" (
                        ...
                        if not "!cell!"==" " if not "!cell!"=="." if not "!cell!"="" (
                            ...
                            set "x!bx!-y!check_y!="
                            if "!cell!"=="%char_gem%" (
                                set /a score=score+difficulty*5+5
                                set /a gems=gems+1
                                ...
                                for /f "tokens=1" %%E in ("!enemy_count!") do (
                                    for /l %%k in (1,1,%%E) do (
    ...
    set /a eb_idx=0
    for /f "tokens=1" %%B in ("!ebullet_count!") do (
        for /l %%i in (1,1,%%B) do (
    ...
    rem Update enemies (they scroll up naturally, move side-to-side every 2 ticks)
    set /a enemy_tick=(!enemy_tick!+1) %% 2

    rem Accuracy thresholds based on difficulty: 1=30%, 2=50%, 3=70%, 4=90%
    set /a acc_thresh=30
    if "!difficulty!"=="2" set /a acc_thresh=50
    if "!difficulty!"=="3" set /a acc_thresh=70
    if "!difficulty!"=="4" set /a acc_thresh=90

    set /a e_idx=0
    for /f "tokens=1" %%B in ("!enemy_count!") do (
        for /l %%i in (1,1,%%B) do (
            set /a ex=!enemy_x[%%i]!
            set /a ey=!enemy_y[%%i]!
            set "x!ex!-y!ey!="
            ...
            set /a next_ex=!ex!
            if "!enemy_tick!"=="0" (
                set /a rand_acc=!random! %% 100
                if !rand_acc! lss !acc_thresh! (
...
set color_stack=
for /f "tokens=1,2,3" %%A in ("!ymin! !ymax! !xmax!") do (
	for /l %%y in (%%A,1,%%B) do (
		for /l %%x in (1,1,%%C) do (
...
set /a ny=y+4
set /a lx=x-3
for /f "tokens=1,2,3,4" %%A in ("!lx! !nx! !y! !ny!") do (
	for /l %%y in (%%C,1,%%D) do (
		for /l %%x in (%%A,1,%%B) do (
...
for /f "tokens=1,2,3" %%A in ("!start! !end! !xmax!") do (
	for /l %%y in (%%A,1,%%B) do (
		for /l %%x in (1,1,%%C) do (
...
if "%difficulty%"=="4" set rows=7&set viewmax=36&set prb=12&set speed=20
